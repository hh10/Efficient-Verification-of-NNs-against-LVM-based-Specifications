...
from models_impl.generic_nets import conv_Nlayer_downscalar, conv_Nlayer_upscalar, LinearNLayerEncoder
from verinet.neural_networks.custom_layers import Reshape

lrelu = nn.LeakyReLU(2e-2)
...
class ClassificationHead(nn.Module):
    def __init__(self, mparams, in_dims):
        ...
        if mparams['source'] == 'hub':
            ...
            self.model = nn.Sequential(*(model_layers[:-1]), nn.Linear(last_layer_in_feats, mparams['cla_args']['num_classes']))
        else:
            self.model = nn.Linear(in_dims, mparams['cla_args']['num_classes'])
